...
class TestRuntimeFtsRebuild:
    ...
    def test_foreign_holder_uninspectable_process_fails_closed(
        self, db, tmp_path, monkeypatch
    ):
        """A process whose fd table is unreadable (different user) is still
        flagged without surfacing any process arguments."""
        ...
        def _listdir(path):
            ...
            if os.fspath(path) == os.fspath(proc_root / "222" / "fd"):
                raise PermissionError("synthetic unreadable fd table")
            return real_listdir(path)
        monkeypatch.setattr(hermes_state.os, "listdir", _listdir)
        monkeypatch.setattr(
            hermes_state,
            "_proc_cmdline_looks_like_hermes",
            lambda pid: pid == 222,
        )

        holders = db._foreign_state_db_holders()
        assert holders == [(222, "uninspectable Hermes process")]
