...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            dm_allowed_raw = self._gate_raw(
                "dm_allow_from", "DISCORD_DM_ALLOWED_USERS"
            )
            if dm_allowed_raw is not None:
                self._dm_allowed_user_ids = {
                    _clean_discord_id(uid)
                    for uid in self._gate_csv_set(dm_allowed_raw)
                    if uid.strip()
                }
...
def _component_check_auth(
    ...
    dm_allowed_env = _scoped_gate_env("DISCORD_DM_ALLOWED_USERS")
    is_dm = getattr(interaction, "guild", None) is None
    if is_dm and dm_allowed_env:
        dm_allowed_users = {
    ...
    # inherited here. Keep these grants after the DM-specific ceiling so a
    # broader allow-all policy cannot reopen private access.
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if dm_allowed_users_cfg is not None:
        if isinstance(dm_allowed_users_cfg, list):
            dm_allowed_users_cfg = ",".join(str(v) for v in dm_allowed_users_cfg)
        seeded_extra["dm_allow_from"] = str(dm_allowed_users_cfg)
        if not _skip_env_bridge and os.getenv("DISCORD_DM_ALLOWED_USERS") is None:
