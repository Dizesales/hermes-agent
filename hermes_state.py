...
def _proc_cmdline_looks_like_hermes(pid: int) -> bool:
    """Classify a proc cmdline without returning or logging its arguments.

    Process arguments may contain credentials.  Keep the raw bytes inside this
    narrow source helper and expose only an allowlisted boolean result.
    """
    try:
        with open(f"/proc/{pid}/cmdline", "rb") as f:
            raw = f.read(4096).lower()
        return any(marker in raw for marker in _HERMES_CMDLINE_MARKERS)
    except OSError:
        return False
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        if _proc_cmdline_looks_like_hermes(pid):
                            holders.append((pid, "uninspectable Hermes process"))
                        continue
                    for fd in fds:
                        try:
